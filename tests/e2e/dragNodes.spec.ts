...
test.describe("node drag", () => {
  ...
  test("edge stroke ends ARROW_CLEARANCE units before the dragged node's border", async ({
    ...
    for (let i = 0; i < inCount; i++) {
      ...
      // endpointOnNode places the endpoint `inset` (= ARROW_CLEARANCE) units
      // past the node border, measured along the center→endpoint ray. Recover
      // that true clearance by walking the same ray out to the rect border —
      // robust to the approach angle (an axis-only projection reads low for
      // steep/diagonal approaches, which vary with the layout).
      const len = Math.hypot(dx, dy);
      const ux = dx / len;
      const uy = dy / len;
      const borderDist = Math.min(
        ux !== 0 ? halfW / Math.abs(ux) : Number.POSITIVE_INFINITY,
        uy !== 0 ? halfH / Math.abs(uy) : Number.POSITIVE_INFINITY,
      );
      const clearance = len - borderDist;
      expect(
        clearance,
        `in-edge ${i} should end ~${ARROW_CLEARANCE} units past the node border; got ${clearance.toFixed(2)} (dx=${dx.toFixed(2)}, dy=${dy.toFixed(2)})`,
      ).toBeGreaterThan(ARROW_CLEARANCE - 2);
