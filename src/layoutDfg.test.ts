...
import type { Dfg, EdgeStats, NodeStats } from "./types.js";
...
describe("layoutDfg — frequency scaling", () => {
  test("node height delta matches maxNodeHeight - minNodeHeight (28 px)", () => {
    const layout = layoutDfg(dfg);
    const maxFreqNode = layout.nodes.reduce((a, b) =>
      a.absoluteFrequency >= b.absoluteFrequency ? a : b,
    );
    const minFreqNode = layout.nodes.reduce((a, b) =>
      a.absoluteFrequency <= b.absoluteFrequency ? a : b,
    );
...
describe("layoutDfg — count modes", () => {
  ...
  test("review_in_progress height on 'case' is strictly less than on 'absolute'", () => {
    ...
    expect(reviewCase?.height).toBeLessThan(reviewAbs?.height ?? 0);
  });
