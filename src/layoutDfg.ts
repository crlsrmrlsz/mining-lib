...
export function layoutDfg(dfg: Dfg, options: LayoutOptions = {}): DfgLayout {
  ...
  // Node height encodes a FIXED metric (case frequency), independent of the
  // active countMode. This keeps the dagre layout — and therefore every node
  // position — identical across count/time modes, so toggling the metric never
  // moves the diagram. The active metric still drives colour, labels, and edge
  // stroke (all post-layout, visual-only) below.
  const heightMetricFor = (node: NodeStats): number => node.caseFrequency;
  const nodeHeightValues = [...dfg.nodes.values()].map(heightMetricFor);
  const edgeFreqs = [...dfg.edges.values()].map((e) => pickCount(countMode, e));
  const heightFor = scaleFor(nodeHeightValues, [minNodeHeight, maxNodeHeight]);
  ...
  for (const node of dfg.nodes.values()) {
    ...
    const height = heightFor(heightMetricFor(node));
