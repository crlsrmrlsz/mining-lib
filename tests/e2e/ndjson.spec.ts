...
// ndjson-input.built.html loads through loadLog (auto-detect) and exposes
...
test("auto-detects NDJSON and renders the diagram (default format)", async ({ page }) => {
  await page.goto("/ndjson-input.built.html?fixture=n5-fixture");
...
test("NDJSON renders the same diagram as CSV (cross-format render parity)", async ({ page }) => {
  await page.goto("/ndjson-input.built.html?fixture=n5-fixture&format=json");
  await waitForReady(page);
  expect(await detected(page)).toBe("ndjson");
  const json = await renderedCounts(page);

  await page.goto("/ndjson-input.built.html?fixture=n5-fixture&format=csv");
  await waitForReady(page);
...
test("auto-detects and renders the larger NDJSON fixture", async ({ page }) => {
  await page.goto("/ndjson-input.built.html?fixture=n1000-realistic&format=json");
  await waitForReady(page);
