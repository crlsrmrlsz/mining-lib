...
async function gotoControlBar(page: Page, fixture = "n5-fixture"): Promise<void> {
  await page.goto(`/control-bar.built.html?fixture=${fixture}&w=900&h=720`);
...
test("Scenario 4 — self-loop edge label sits outside the source node's bounding box", async ({
  ...
  const info = await page.evaluate(() => {
    ...
    // control-bar.built.html exposes the UMD bundle as `MiningLib` (see
    // example/control-bar.built.html). Feed the existing handle a
